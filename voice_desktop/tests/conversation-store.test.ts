...
import { conversationReducer, getPhaseCopy, initialConversationState, shouldStopPlaybackForAsr } from '../src/features/conversation/conversation-store';
...
describe('ASR playback interruption', () => {
  it('ignores empty text and interrupts once for the first non-empty partial or final', () => {
    expect(shouldStopPlaybackForAsr('   ', 'message-1', 'message-0')).toBe(false);
    expect(shouldStopPlaybackForAsr('你', 'message-1', 'message-0')).toBe(true);
    expect(shouldStopPlaybackForAsr('你好', 'message-1', 'message-1')).toBe(false);
    expect(shouldStopPlaybackForAsr('你好', 'message-2', 'message-1')).toBe(true);
  });
});
