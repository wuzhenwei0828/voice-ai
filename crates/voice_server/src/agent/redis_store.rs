...
mod tests {
    ...
    /// 集成测试需要真实 Redis —— 默认 ignore，需要时手动跑：
    /// `REDIS_URL=redis://127.0.0.1:6379/ cargo test -p voice_server redis_store -- --ignored`
    #[tokio::test]
    #[ignore]
    async fn integration_round_trip() {
        ...
        let store = RedisStore::connect_with_prefix(
            &url,
            3,
            "voice:test:memory:".to_string(),
            60,
        )
        .await
        .expect("redis must be reachable");

        let sid = "integration-sid";
        ...
        store
            .append(
                sid,
                Message {
                    role: crate::agent::memory::Role::User,
        ...
        store
            .append(
                sid,
                Message {
                    role: crate::agent::memory::Role::Assistant,
        ...
        let h = store.history(sid).await;
        assert_eq!(h.len(), 3);
        assert_eq!(h[0].content, "u1");
        assert_eq!(h[1].content, "a1");
        assert_eq!(h[2].content, "u2");
        ...
        store
            ...
            .await;
        let h2 = store.history(sid).await;
        assert_eq!(h2.len(), 3);
        assert_eq!(h2[0].content, "a1");
        assert_eq!(h2[2].content, "a2");

        store.clear(sid).await;
        assert_eq!(store.len(sid).await, 0);
    }
}